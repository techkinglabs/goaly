...
public interface TargetHistoryRepository extends JpaRepository<TargetHistory, Long> {
    ...
    @Query("select h from TargetHistory h where h.goalId = :goalId and (:excludeId is null or h.id <> :excludeId) and (:date is null or h.validFrom <= :date) and (:date is null or h.validTo is null or h.validTo >= :date) order by h.validFrom desc")
